...
# AutoHotkey KEY_IGNORE: события с этим dwExtraInfo не запускают его hotkey.
# Это предотвращает ложное переключение языка в LangBarXX при fallback Ctrl+C.
AUTOHOTKEY_IGNORE_MARKER = 0xFFC3D44F
...
def _send_scancode(scan_code: int, key_up: bool = False, extra_info: int = 0) -> bool:
    flags = KEYEVENTF_SCANCODE | (KEYEVENTF_KEYUP if key_up else 0)
    keyboard_input = KEYBDINPUT(0, scan_code, flags, 0, extra_info)
...
def send_left_ctrl_c() -> bool:
    ...
    ok &= _send_scancode(
        ctrl_scan,
        key_up=False,
        extra_info=AUTOHOTKEY_IGNORE_MARKER,
    )
